...
4. **Source coverage:** Which source documents inform which sections
5. **Gaps identified:** Topics the guiding questions ask about but the source material doesn't cover (these will become `open_questions` or `assumptions`)
...
**Sources:**
- Introduction transcript (25/02) → Sections 1, 2, 5
- Briefing Document → Sections 2, 3, 4
- Check-in transcript (24/02) → Section 2
...
2. **Every `points` entry in `core_content` must trace to a source.** If it doesn't, move it to `assumptions`. No exceptions.
...
5. **Sources must be specific.** Include document name and timestamp/page/section when available:
   ```yaml
   sources:
     - doc: "introduction_transcript_20260225.txt"
       details: "~08:31 - Chris on campaign hierarchy"
   ```
...
1. Save the file as `MM-[ID]-[title-kebab-case].yml` in the current working directory.
...
- [ ] Every `core_content` point has a source, or is in `assumptions`
