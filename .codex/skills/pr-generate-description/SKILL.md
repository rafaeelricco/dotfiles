...
description: >
  ...
  updating any GitHub pull request body, including when the user or Codex will
...
- Creating or opening a pull request through the GitHub app, GitHub UI, or
  `gh pr create`.
...
4. Ask structured formatting questions with Codex `request_user_input` when that
   tool is available; otherwise ask concise plain-text questions.
...
After receiving motivation, ask these formatting decisions. Prefer the Codex
`request_user_input` tool when it is available:
...
- PR will be created in this session: do not write `pr-description.markdown`.
  Pass the description to the GitHub app or `gh pr create` via body field or
  body file.
- PR already exists: do not write `pr-description.markdown` by default. Present
  the full description in chat and update the PR only if the user asks.
