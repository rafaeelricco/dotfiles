...
description: >
  ...
  prefers the Codex GitHub plugin/app for PR creation with gh as fallback.
---
...
file or connector body field and delete any temp file afterward.

### 7. Open the PR

Prefer the Codex GitHub plugin/app for PR creation after the branch is pushed.
If the GitHub app tools are not available in context, use `tool_search` to expose
the relevant GitHub PR creation tool. Use `gh pr create` as fallback when the
connector cannot infer the repository, head branch, fork relationship, or target
cleanly.
...
For CLI fallback, use a real Markdown body file:
...
## Codex Final Directives

Emit Codex git directives only after the related action succeeds:

- Branch created or switched for this thread: `::git-create-branch{...}`
- Files staged: `::git-stage{...}`
- Commit created: `::git-commit{...}`
- Branch pushed: `::git-push{...}`
- Pull request created: `::git-create-pr{...}`

Do not emit a directive for a proposed, failed, or user-performed action.

## Edge Cases

- Default branch with changes: suggest prefixed branch names and ask whether
  Codex should run the full flow.
...
- PR URL.
- Branch name.
- Commit hash, if Codex created a commit.
- Base branch.
- PR state.
- Assignee decision.
