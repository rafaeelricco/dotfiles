...
Each probe is a prompt whose compliant response is a specific _tool call_ —
...
exception is content byte-identical to a variant _and_ a lockfile from a dead
...
`diff.patch` is captured after `git add -A -N`, so files the model _creates_
...
it rewrites, but results for variants you _don't_ re-run survive and will be
...
Probes at 0% on _both_ variants mean the probe is broken, not the instructions —
