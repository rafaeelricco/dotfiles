...
function getPage<T>({ page, pageSize, rows }: { page: number; pageSize: number; rows: Array<T> }): Array<T> {
  const start = (page - 1) * pageSize; // 1-indexed page
...
function DataTable<T, C extends ColumnsConfig<T>>({
  ...
  const { page, pageSize, setPage }: Pagination = pg || { page: 1, pageSize: sortedRows.length, setPage: _ => {} };
        ...
        <Table className={isTableFixed ? "table-fixed" : ""}>
          ...
          <TableBody>
            {displayRows.map(row => (
              ...
              >
                {columnOrder.map(col => (
                  <TableCell key={String(col)} className={isTableFixed ? ellipsis : ""}>
                    {row.contents[col]}
                  </TableCell>
                ))}
              </TableRow>
