...
function DataTable<T, C extends ColumnsConfig<T>>({
  ...
  const sortFun : SortFun<T> =
    ...
    ? (columns[sortState.column]?.values.sortFun || noSorting)
    : sortState.sorting === "decreasing"
    ? reverseSort(columns[sortState.column]?.values.sortFun || noSorting)
    : noSorting
    ...
    <div className="grid gap-3">
      <div className="rounded-md border">
        <Table className={isTableFixed ? "table-fixed" : ""}>
          ...
          {displayRows.map((row, ix) =>
            <TableRow key={ix} className={cn(row.className, row.onClick ? 'cursor-pointer hover:bg-muted/30' : '')} onClick={() => row.onClick?.(row.value)}>
      ...
      {hasPagination &&
        <div className="flex flex-cols justify-between items-center">
            <p className="text-sm text-muted-foreground">{sortedRows.length} entries. Page {page} of {maxPage}</p>
            <div className="grid grid-cols-2 gap-1">
              <Button variant="outline" size="sm" onClick={prevPage} disabled={page === 0}>Previous</Button>
              <Button variant="outline" size="sm"onClick={nextPage} disabled={page === maxPage}>Next</Button>
            </div>
        </div>
      }
    </div>
