...
# Mover estas configurações para o início do arquivo depois das definições de função
# para garantir que execute antes de qualquer coisa
$ConfirmPreference = 'None'
$PSDefaultParameterValues['Exit-PSSession:Confirm'] = $false

# Limpar arquivos antigos e restaurar último diretório imediatamente após as definições de funções
Clear-OldDirectoryFiles
Set-LocationFromTabDirectories

# Remover inicialização duplicada que está no final do arquivo
# Movendo o bloco try/catch para ajudar na inicialização
try {
...
#>
# Armazenar a função de prompt atual (que pode ter sido definida por Oh My Posh ou outros módulos)
$FUNCTION:ORIGINAL_PROMPT = $FUNCTION:prompt

# Redefinir a função prompt para incluir o salvamento de diretório
function prompt {
